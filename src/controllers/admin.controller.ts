...
type ProposalStatus = 'submitted' | 'under_review' | 'approved' | 'rejected' | 'revision_requested';

class AdminController {
  // Get all proposals with pagination and filtering
  getAllProposals = asyncHandler(async (req: Request, res: Response<IProposalResponse>): Promise<void> => {
  ...
  // Get proposal by ID
  getProposalById = asyncHandler(async (req: Request, res: Response<IProposalResponse>): Promise<void> => {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      throw new UnauthorizedError('You do not have permission to access this resource');
    }
  ...
  updateProposalStatus = asyncHandler(async (req: Request, res: Response<IProposalResponse>): Promise<void> => {
    const { id } = req.params;
    ...
    try {
      ...
      logger.error('Failed to send status update email:', error instanceof Error ? error.message : 'Unknown error');
    }
  ...
  getProposalStatistics = asyncHandler(async (req: Request, res: Response<IStatisticsResponse>): Promise<void> => {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      throw new UnauthorizedError('You do not have permission to access this resource');
    }
