...
agenda.define('generate AI review', async (job: Job<AIReviewJobData>) => { // Type the job parameter
  const { proposalId } = job.attrs.data;
  if (proposalId) {
    await generateAIReviewForProposal(proposalId);
  } else {
    console.error('Agenda job "generate AI review" received without proposalId.');
